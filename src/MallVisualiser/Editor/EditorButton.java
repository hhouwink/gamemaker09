...
public class EditorButton extends Button {
	...
	private final String[] myFileLocations = new String[] { // TODO: Hard-coded
															// file locations.
															// Remove
															// eventually.
	...
	public EditorButton(ButtonType type, Game game, Pixmap pix, int x, int y,
			WorldIO worldIO) {
		super(pix, x, y);
		this.type = type;
		this.game = game;
		...
		if (type == ButtonType.NewMap) {
			newMapDialog = new NewMapDialog((Frame) game.getContainer(),
					worldIO);
		}
		if (type == ButtonType.ResizeMap) {
			resizeMapDialog = new ResizeMapDialog((Frame) game.getContainer(),
					worldIO);
		}
		if (type == ButtonType.toMenu) {
			toMenuDialog = new BackToMenuDialog((Frame) game.getContainer(),
					this);
		}
	...
	public static enum ButtonType {
		NewMap, ResizeMap, SaveMap, LoadMap, toMenu
	}
